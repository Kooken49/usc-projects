...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // ADMIN LOGIN
    $stmt = $conn->prepare("SELECT * FROM Admin WHERE AdminEmail = ? AND password = ? AND IsActive = TRUE");
    ...
    // TEACHER LOGIN
    $stmt = $conn->prepare("SELECT * FROM Teacher WHERE TeacherEmail = ? AND Password = ? AND IsActive = TRUE");
    $stmt->bind_param("ss", $email, $password);
    $stmt->execute();
    ...
    // STUDENT LOGIN
    $stmt = $conn->prepare("SELECT * FROM Student WHERE StudentEmail = ? AND Password = ? AND IsActive = TRUE");
    $stmt->bind_param("ss", $email, $password);
    $stmt->execute();
    ...
    // If all failed
    $error = "Invalid email, password, or your account has been deactivated.";
}
?>

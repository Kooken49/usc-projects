...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // ADMIN LOGIN
    $stmt = $conn->prepare("SELECT * FROM Admin WHERE AdminEmail = ? AND IsActive = TRUE");
    ...
    // TEACHER LOGIN
    $stmt = $conn->prepare("SELECT * FROM Teacher WHERE TeacherEmail = ? AND IsActive = TRUE");
    $stmt->bind_param("s", $email);
    $stmt->execute();
    ...
    // STUDENT LOGIN
    $stmt = $conn->prepare("SELECT * FROM Student WHERE StudentEmail = ? AND IsActive = TRUE");
    $stmt->bind_param("s", $email);
    $stmt->execute();

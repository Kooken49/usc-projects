...
$errorMessage = ""; // For HTML display

function is_valid_password($password) {
    return strlen($password) >= 8 &&
           preg_match('/[A-Z]/', $password) &&
           preg_match_all('/[a-zA-Z]/', $password) >= 4 &&
           preg_match_all('/[0-9]/', $password) >= 4 &&
           preg_match('/[!@#$%^&*(),.?":{}|<>]/', $password);
}
    ...
    if ($type === 'admin') {
        if (isset($_POST['add'])) {
            ...
            if (!empty($_POST['password'])) {
                if ($_POST['password'] !== $_POST['confirm_password']) {
                    $errorMessage = "Passwords do not match.";
                } elseif (!is_valid_password($_POST['password'])) {
                    $errorMessage = "Password must be at least 8 characters, with 4 letters, 4 numbers, 1 uppercase letter, and 1 special character.";
                } else {
                    $hashedPassword = password_hash($_POST['password'], PASSWORD_DEFAULT);
                    $stmt = $conn->prepare("UPDATE Admin SET AdminName = ?, AdminEmail = ?, password = ? WHERE id = ?");
                    $stmt->bind_param("sssi", $_POST['name'], $_POST['email'], $hashedPassword, $_POST['id']);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
        ...
        if (isset($_POST['add'])) {
            ...
            if ($password !== $confirm) {
                ...
                $stmt->bind_param("sssss", $_POST['name'], $_POST['email'], $_POST['num'], $hashedPassword, $_POST['level']);
                $stmt->execute();
                $stmt->close();
            }
        } elseif (isset($_POST['edit'])) {
            if (!empty($_POST['password'])) {
                if ($_POST['password'] !== $_POST['confirm_password']) {
                    ...
                    $stmt->bind_param("sssssi", $_POST['name'], $_POST['email'], $_POST['num'], $_POST['level'], $hashedPassword, $_POST['id']);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
        ...
        if (isset($_POST['add'])) {
            ...
            if ($password !== $confirm) {
                ...
                $stmt->bind_param("ssss", $_POST['name'], $_POST['email'], $_POST['num'], $hashedPassword);
                $stmt->execute();
                $stmt->close();
            }
        } elseif (isset($_POST['edit'])) {
            if (!empty($_POST['password'])) {
                if ($_POST['password'] !== $_POST['confirm_password']) {
                    ...
                    $stmt->bind_param("ssssi", $_POST['name'], $_POST['email'], $_POST['num'], $hashedPassword, $_POST['id']);
                    $stmt->execute();
                    $stmt->close();
                }
            } else {
...
$idField = $type === 'admin' ? 'id' : ($type === 'student' ? 'StudentID' : 'TeacherID');
$nameField = $type === 'admin' ? 'AdminName' : ($type === 'student' ? 'StudentName' : 'TeacherName');
$emailField = $type === 'admin' ? 'AdminEmail' : ($type === 'student' ? 'StudentEmail' : 'TeacherEmail');
$numberField = $type === 'student' ? 'StudentNum' : ($type === 'teacher' ? 'TeacherNum' : null);
...
// Search term (applies to all)
if (!empty($search)) {
    $searchLike = "%$search%";
    $where[] = "($idField LIKE ? OR $nameField LIKE ? OR $emailField LIKE ?" . ($numberField ? " OR $numberField LIKE ?" : "") . ")";
    $types .= $numberField ? "ssss" : "sss";
    $params[] = &$searchLike;
    $params[] = &$searchLike;
    $params[] = &$searchLike;
    if ($numberField) $params[] = &$searchLike;
}

// Filter level (only for students)
if ($type === 'student' && !empty($selectedLevel)) {
    $where[] = "Level = ?";
    $types .= "s";
  ...
  <div class="ml-64 min-h-screen flex flex-col">
    ...
    <main class="p-6 flex-1 overflow-auto">
      ...
      <form method="POST" class="mb-10 space-y-2">
        ...
        <button
          type="button"
          onclick="document.getElementById('add_form').scrollIntoView({ behavior: 'smooth' });"
          class="bg-green-500 text-white px-3 py-1 rounded text-sm font-bold hover:bg-green-600"
        >
          ＋Add New Admin<?= ucfirst($type) ?>
        </button>
        <div class="overflow-x-auto">
          <table class="min-w-full border border-gray-400 rounded text-base">
                ...
                <form method="POST">
                  <tr class="<?= $isActive ? '' : 'bg-gray-100' ?>">
                    ...
                    <td class="px-2 py-2 border border-gray-300 w-[10%]">
                      ...
                      <?php else: ?>
                        <input type="hidden" name="id" value="<?= $rowID ?>">
                        <button name="reactivate" class="bg-green-500 text-white text-sm font-bold px-2 py-1 rounded hover:bg-green-600 w-full">Reactivate</button>
                      <?php endif; ?>
                    </td>
                  </tr>
                </form>
            ...
            <!-- Add New User Form -->
            <form method="POST" class="mb-6 p-4 border rounded bg-white shadow">
              <a id="add_form"></a>
              <h2 class="text-lg font-bold mb-2">Add New <?= ucfirst($type) ?></h2>
              <div class="grid grid-cols-<?= $type === 'admin' ? '2' : '3' ?> gap-4">
                ...
                <?php if ($type !== 'admin'): ?>
                  <input name="num" placeholder="Contact Number" class="border p-2 rounded" required>
                <?php endif; ?>
                <?php if ($type === 'student'): ?>
                  <select name="level" class="border p-2 rounded" required>
                    <option value="Basic">Basic</option>
                    <option value="Advanced">Advanced</option>
                  </select>
                <?php endif; ?>
                <input name="password" type="password" placeholder="Password" class="border p-2 rounded" required>
                <input name="confirm_password" type="password" placeholder="Confirm Password" class="border p-2 rounded" required>
              </div>
              <div class="mt-4">
                <button type="submit" name="add" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Add <?= ucfirst($type) ?></button>
              </div>
            </form>
        </div>
      </form>

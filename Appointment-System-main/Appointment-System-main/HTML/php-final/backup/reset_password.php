...
if (!isset($_SESSION['reset_email']) || !isset($_SESSION['verified_token'])) {
    echo "Unauthorized access. Please verify your token first. <a href='verify_token.php'>Go back</a>.";
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($password1 !== $password2) {
        ...
        $stmt = $conn->prepare("SELECT * FROM password_reset_tokens WHERE Email = ? ORDER BY CreatedAt DESC LIMIT 1");
        ...
        if ($res->num_rows === 1) {
            ...
            echo "Reset token not found. Try again.";
        }
    }
}
?>

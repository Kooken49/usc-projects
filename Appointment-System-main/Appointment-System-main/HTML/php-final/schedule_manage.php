...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['approve']) && isset($_POST['TeacherID'])) {
        ...
        $conn->query("INSERT INTO Notification (SenderType, Title, Message) VALUES ('System', '$title', '$message')");
...
$allRequests = $conn->query("
    SELECT 
        ...
        s.StudentName, 
        sb.PreferredDate, 
        sb.PreferredTimeStart, 
        sb.PreferredTimeEnd, 
        sb.Status,
        (
            SELECT t.TeacherName 
            FROM ScheduleSlot ss 
            JOIN Teacher t ON ss.TeacherID = t.TeacherID
            WHERE ss.StudentID = sb.StudentID 
              AND ss.Date = sb.PreferredDate 
              AND ss.TimeStart = sb.PreferredTimeStart 
            LIMIT 1
        ) AS TeacherName
    FROM StudentBooking sb
    JOIN Student s ON sb.StudentID = s.StudentID
    ORDER BY sb.PreferredDate DESC
");
        ...
        <div class="overflow-x-auto bg-white shadow rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              <!-- All Schedule Requests Table -->
        <div class="mt-10 overflow-x-auto bg-white shadow rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              <tr>
                ...
                <td class="px-6 py-4 text-sm"><?= $row['PreferredDate'] ?></td>
                <td class="px-6 py-4 text-sm"><?= $row['PreferredTimeStart'] ?> - <?= $row['PreferredTimeEnd'] ?></td>
                <td class="px-6 py-4 text-sm"><?= $row['TeacherName'] ? htmlspecialchars($row['TeacherName']) : '—' ?></td>
              ...
              <tr><td colspan="5" class="text-center py-4 text-gray-500">No schedule requests found.</td></tr>
              <?php endif; ?>
            </tbody>
          </table>
        </div>

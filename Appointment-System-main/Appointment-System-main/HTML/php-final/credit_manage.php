...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['add_credit'])) {
    ...
    if ($plan && isset($creditMap[$plan])) {
        ...
        $stmt = $conn->prepare("INSERT INTO BookingCredits (StudentID, CreditAmount, Plan, ExpiryDate, IssuedAt) VALUES (?, ?, ?, ?, CURDATE())");
...
if (isset($_POST['update_credit'])) {
    ...
    $stmt = $conn->prepare("UPDATE BookingCredits SET StudentID = ?, Plan = ?, CreditAmount = ?, ExpiryDate = ? WHERE CreditID = ?");
    $stmt->bind_param("isisi", $studentId, $plan, $creditAmount, $expiryDate, $creditId);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['delete_credit'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM BookingCredits WHERE CreditID = ?");
          ...
          <table class="min-w-full table-auto border border-gray-300">
            ...
            <tbody>
              ...
              while ($s = $students->fetch_assoc()) {
                  $studentOptions[$s['StudentID']] = $s['StudentName'];
              }
              $credits->data_seek(0); // reset pointer
              while ($row = $credits->fetch_assoc()):
              ?>
              <form method="POST">
              <tr class="text-center">
                <input type="hidden" name="CreditID" value="<?= $row['CreditID'] ?>">
                
                <!-- Student -->
                <td class="border px-4 py-2">
                  <select name="StudentID" class="border rounded p-1 text-sm">
                    <?php foreach ($studentOptions as $id => $name): ?>
                      <option value="<?= $id ?>" <?= $id == $row['StudentID'] ? 'selected' : '' ?>><?= htmlspecialchars($name) ?></option>
                    <?php endforeach; ?>
                  </select>
                </td>
                ...
                <!-- Plan -->
                <td class="border px-4 py-2">
                  <select name="Plan" class="border rounded p-1 text-sm">
                    <?php foreach ($plans as $key => $plan): ?>
                      <option value="<?= $key ?>" <?= $key == $row['Plan'] ? 'selected' : '' ?>>
                        <?= $plan['label'] ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                ...
                <!-- Credits -->
                <td class="border px-4 py-2">
                  <input type="number" name="CreditAmount" value="<?= $row['CreditAmount'] ?>" class="border p-1 w-20 text-sm text-center">
                </td>

                <!-- Issued At (readonly) -->
                <td class="border px-4 py-2"><?= date('Y-m-d', strtotime($row['IssuedAt'])) ?></td>

                <!-- Expiry Date -->
                <td class="border px-4 py-2">
                  <input type="date" name="ExpiryDate" value="<?= $row['ExpiryDate'] ?>" class="border p-1 rounded text-sm">
                </td>

                <!-- Actions -->
                <td class="border px-4 py-2 space-x-1">
                  <button name="update_credit" class="bg-yellow-500 text-white px-2 py-1 rounded text-sm hover:bg-yellow-600">Update</button>
                  <button name="delete_credit" onclick="return confirm('Delete this credit entry?')" class="bg-red-500 text-white px-2 py-1 rounded text-sm hover:bg-red-600">Delete</button>
                </td>
              </tr>
              </form>
              <?php endwhile; ?>
